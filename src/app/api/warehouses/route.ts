import { z } from 'zod';
import { demoMode } from '@/lib/config';
import { mockWarehouses } from '@/lib/mock-data';
import { fail, ok } from '@/lib/api/respond';
import { getAuthContext } from '@/lib/auth/server';

const reservationSchema = z.object({
  warehouseId: z.string().min(1),
  reservedAreaM2: z.number().positive(),
  startDate: z.string().min(8),
  endDate: z.string().min(8),
});

export async function GET() {
  if (demoMode) return ok(mockWarehouses);
  const { supabase, user } = await getAuthContext();
  if (!supabase) return fail('BACKEND_NOT_CONFIGURED', 'Warehouse service is not configured', 503);
  if (!user) return fail('UNAUTHENTICATED', 'Authentication is required', 401);
  const { data, error } = await supabase.from('warehouses').select('*').order('name');
  if (error) return fail('DATABASE_ERROR', 'Unable to load warehouses', 500);
  return ok(data ?? []);
}

export async function POST(request: Request) {
  const parsed = reservationSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) return fail('VALIDATION_ERROR', parsed.error.issues[0]?.message ?? 'Invalid reservation', 422);
  if (demoMode) return ok({ ...parsed.data, status: 'pending', mode: 'demo' }, 201);
  return fail('NOT_IMPLEMENTED', 'Warehouse reservations require the warehouse migration', 501);
}
